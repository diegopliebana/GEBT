...
double evaluate(const string &phenotype, const GEMap &mapper, const qGA &pop, const size_t &gen,
	...
	size_t iSeed = gen * 5 + 1;
	size_t fSeed = gen * 5 + 5;
	// 1) Write phenotype to file;
	ofstream phenoFile;
	stringstream pFile;
	pFile << "../BTs/phenotype-noAstar-change5-" << pop.getRandomSeed() << "-" << parIndex << ".xml";
	...
	for(size_t seed = iSeed ; seed <= fSeed; ++seed){
		...
		sysCall << "if [ -f ../BTs/fitness-noAstar-change5-" << pop.getRandomSeed() << "-" << parIndex << ".txt ] ;"
			<< " then rm ../BTs/fitness-noAstar-change5-" << pop.getRandomSeed() << "-" << parIndex << ".txt ; fi; "
		...
		sysCall
			<< " -if ../../BTs/phenotype-noAstar-change5-" << pop.getRandomSeed() << "-" << parIndex << ".xml"
			<< " -of ../../BTs/fitness-noAstar-change5-" << pop.getRandomSeed() << "-" << parIndex << ".txt"
			<< " -os /dev/null >> /dev/null";
		system(sysCall.str().c_str());
		// 4) Recover fitness;
		stringstream fFile;
		fFile << "../BTs/fitness-noAstar-change5-" << pop.getRandomSeed() << "-" << parIndex << ".txt";
...
int main(int argc, char **argv){
	...
	for(size_t gen = 0; gen < pop.getMaxGens(); ++gen){
		...
		bestPhenoFile << "../BTs/best-noAstar-change5-" << pop.getRandomSeed() << "-" <<
			gen + 1 << "-" <<

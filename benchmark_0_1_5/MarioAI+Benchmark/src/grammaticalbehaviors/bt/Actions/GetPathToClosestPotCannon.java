 ...
 * @author Diego
 */
public class GetPathToClosestPotCannon extends BTLeafNode{

    private Vector<Integer> m_forbidden;

    public GetPathToClosestPotCannon(BTNode a_parent)
    {
        super(a_parent);
    ...
    public void step() throws IncorrectNodeException 
    {
        ...
        //######
        /*if(marioNodeId <  10210027 || marioNodeId >  10210028) //Input here the positions
        {
            m_nodeStatus = BTConstants.NODE_STATUS_FAILURE;
            m_parent.update(m_nodeStatus);
            return;
        }else if(mario.getCurrentPath().m_metadata == Path.PATH_POTCANNON)
        {
        ...
        if(levelGraph.getNumNodes() == 0 || (curPath != null && curPath.m_metadata == Path.PATH_POTCANNON))
        {
        ...
        int idNodeDest = levelGraph.getClosestPotCannonNode(marioNodeId, m_forbidden);
        ...
        else
        {
                 ...
                 pathToDestination.m_cost < Integer.MAX_VALUE)
            {
                ...
                pathToDestination.m_metadata = Path.PATH_POTCANNON;

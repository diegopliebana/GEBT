 ...
 * @author Diego
 */
public class GetPathToRightMost extends BTLeafNode{

    private Vector<Integer> m_forbidden;

    public GetPathToRightMost(BTNode a_parent)
    {
        super(a_parent);
    ...
    public void step() throws IncorrectNodeException 
    {
        ...
        int idNodeDest = levelGraph.getRightMostNodeClose(marioNodeId, m_forbidden);
        if(idNodeDest == -1)
        {
            //No destination node
            m_nodeStatus = BTConstants.NODE_STATUS_FAILURE;
        }
        else if(marioNodeId == idNodeDest)
        {
            mario.setOnRightMostFlag(true);
        }
        else
        {
                 ...
                 pathToDestination.m_cost < Integer.MAX_VALUE)
            {
                ...
                m_forbidden.clear();
                mario.setOnRightMostFlag(false);
            }
            else
            {
                if(mario.inTrap())

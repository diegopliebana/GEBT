 ...
 * @author Diego
 */
public class GoGapOnRight extends BTLeafNode{

    int m_stepsInObjective;
    int m_nopTime;
    private final int MAX_OBJ_STEPS = 15;
    

    public GoGapOnRight(BTNode a_parent)
    {
        super(a_parent);
    ...
    public void step() throws IncorrectNodeException 
    {
        ...
        //if((objective == -1) || ((marioPos != objective) && (objective != -1)))
        ...
        boolean thereIsGapOnLeft = false;
        int gapPosOnLeft = -1;
        if((objective == -1) || (marioPos != objective) || forceRecalculate)
        {
           int thisGap = -1;

           // for(int i = gaps.size()-1;!thereIsGap && i >=0; --i)
            for(int i = 0;!thereIsGap && i < gaps.size(); ++i)
            {
                ...
                {
                    thereIsGapOnLeft = true;
                    gapPosOnLeft = thisGap;
                }
            }
        }
        ...
        else if(forceRecalculate && !thereIsGap && thereIsGapOnLeft)
        {
            thereIsGap = true;
            objective = gapPosOnLeft;
            mario.setObjective(gapPosOnLeft);
        }

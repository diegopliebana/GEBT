 ...
 * @author Diego
 */
public class GetPathToGround extends BTLeafNode{

    private Vector<Integer> m_forbidden;

    public GetPathToGround(BTNode a_parent)
    {
        super(a_parent);
    ...
    public void step() throws IncorrectNodeException 
    {
        ...
        //######
        /*if(marioNodeId < 11110030 || marioNodeId > 11110035) //Input here the positions
        {
            m_nodeStatus = BTConstants.NODE_STATUS_FAILURE;
            m_parent.update(m_nodeStatus);
            return;
        }else if(mario.getCurrentPath().m_metadata == Path.PATH_GROUNDMOST)
        {
        ...
        if(levelGraph.getNumNodes() == 0 || (curPath != null && curPath.m_metadata == Path.PATH_GROUNDMOST))
        {
        ...
        int idNodeDest = levelGraph.getGroundMostNodeClose(marioNodeId, m_forbidden);
        ...
        else
        {
                 ...
                 pathToDestination.m_cost < Integer.MAX_VALUE)
            {
                ...
                pathToDestination.m_metadata = Path.PATH_GROUNDMOST;
